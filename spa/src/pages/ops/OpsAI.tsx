    ...
    <div>
      ...
      <div className="grid cols-2">
        <div className="card"><h3>Estado</h3>
          ...
          <p style={{ fontSize: 13 }}><b>Fuente de la clave:</b> {st.source === 'env' ? 'variable de entorno DEEPSEEK_API_KEY' : st.source === 'param' ? 'parámetro del sistema' : st.source === 'record' ? 'registro de integración' : 'ninguna'}{st.key_hint && ` (${st.key_hint})`}<br /><b>Modelos por nivel:</b><br />· Rápido (transcripciones, extracción, clasificación, resúmenes preliminares): <b>{st.models?.fast}</b><br />· Profundo (resumen ejecutivo final, análisis de sesiones, requerimientos técnicos): <b>{st.models?.deep}</b><br />· Visión (capturas, diagramas, PDFs escaneados, evidencia): <b>{st.models?.vision}</b><br /><b>Endpoint:</b> {st.base_url}</p>
