...
4. Cron cada 10 minutos (`aq.google.sync.cron_sync`): Calendar → reuniones de proyecto; Meet API → transcripciones a la reunión + propuestas del copiloto; Drive → notas de Gemini; Gmail → bandeja enrutada (`aq.google.message`) según **reglas** (`aq.google.rule`, precargadas) y copiloto DeepSeek: Administración (facturación, cobranza, pagos, legal, RH, prospectos) u Operaciones (solicitudes, incidentes, reuniones). Conversión con un clic o automática por regla; los correos procesados reciben la etiqueta `Alphaqueb/Procesado`.
   **Compuerta de dominios** (`aq_google.only_project_domains=1`, por defecto): solo se analiza —adjuntos, reglas por palabras clave y copiloto— el correo cuyo dominio (De/Para/CC) está en *Dominios de correo del cliente* de un proyecto activo; también pasan las notas de Meet/Gemini y el correo que una regla identifica por remitente/destinatario (`Remitente contiene` / `Destinatario contiene`, p. ej. un proveedor). Todo lo demás queda en la bandeja como *Ignorado* con el dominio detectado y sin consumir tokens; tras agregar el dominio al proyecto, la acción **Reanalizar** lo procesa.
...
* **Operaciones → Sesiones**: eliges proyecto + tipo (catálogo `aq.ops.session.type`: Daily Sync, Retrospectiva, Seguimiento, Capacitación, Entrega, Validación, Pruebas/UAT, Kickoff, Revisión, Emergencia, Producción) → folio automático, evento de Calendar con **Meet**, invitación a equipo interno + contactos del cliente + grupo de correo + extras (`sendUpdates=all`), agenda del tipo.
* Al llegar la transcripción (Meet API / notas de Gemini / correo) se dispara `process_with_ai`: **resumen ejecutivo estructurado** (objetivo, resumen, temas, decisiones, acuerdos, pendientes del cliente, riesgos, siguientes pasos, próxima sesión) generado por DeepSeek, **documento en tu plantilla de Google Docs** (busca "Plantilla…Sesión/Resumen/Minuta" en Drive o crea una con marcadores `{{RESUMEN}}`, etc.; parámetro `aq_google.session_template_doc_id`), **correo con el resumen** al equipo y Dirección, y **actividades creadas automáticamente** desde los acuerdos (`aq_ops.auto_confirm_agreements=1`).
* **Mapa de sesiones**: todas las sesiones por proyecto o cronológicas, con folio, tipo, estado, IA, acuerdos y enlaces (Meet/Doc); descarga a Google Sheets. **Importar histórico** trae los últimos 12 meses de Calendar (idempotente), fija prefijo/secuencia por proyecto y deja como tarea futura "Procesar transcripciones de sesiones históricas" en cada proyecto.

## Documentos formales y biblioteca de prompts
* **Constructor documental de marca** (`aq.doc.builder`): copia la plantilla membretada (A4, logo y pie de contacto) e inserta el contenido con el sistema de diseño de Alphaqueb — Bebas Neue en el título, Oxanium en etiquetas y encabezados de sección (morado #723FA0, filete lima), Roboto en el cuerpo justificado, fichas y tablas nativas de Google Docs (encabezado lima, bordes suaves) y nota de confidencialidad al cierre.
